_poetry_8297617dcfab8f8b_complete()
{
    ...
    if [[ $cur == $com ]]; then
        coms="about add build 'cache clear' 'cache list' check config 'debug info' 'debug resolve' 'debug tags' dynamic-versioning 'dynamic-versioning enable' 'dynamic-versioning show' 'env activate' 'env info' 'env list' 'env remove' 'env use' help init install list lock new publish 'python install' 'python list' 'python remove' remove run search 'self add' 'self install' 'self lock' 'self remove' 'self show' 'self show plugins' 'self sync' 'self update' show 'source add' 'source remove' 'source show' sync update version"
...
complete -o default -F _poetry_8297617dcfab8f8b_complete poetry
complete -o default -F _poetry_8297617dcfab8f8b_complete /home/jacdev/projects/dotfiles/.config/bash/completions/poetry
